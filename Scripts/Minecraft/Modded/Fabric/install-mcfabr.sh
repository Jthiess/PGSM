...
if [ -z "$MC_VERSION" ]; then
    echo "ERROR: mc_version argument is required for Fabric."
    exit 1
fi
...
echo "Installing Fabric for MC ${MC_VERSION}..."
mkdir -p /PGSM
cd /PGSM
...
if [ -n "$FABRIC_VERSION" ]; then
  echo "Using Fabric loader version: $FABRIC_VERSION"
...
# Step 9: Create wrapper script that monitors the tmux session
mkdir -p /opt/pgsm
...
[Unit]
Description=Proxmox Game Server Manager - Fabric
...
ExecStart=/opt/pgsm/run.sh $STARTUP_COMMAND
